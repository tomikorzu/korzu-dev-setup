...
Next.js + MUI + GSAP starter. Scaffolded standalone by `create-korzu-app --frontend`, or as
`apps/web` when combined with other layers. Treat everything here as library-quality code, not
a one-off app — it's what every new project actually starts from.
